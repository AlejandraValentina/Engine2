...
- **Ecuación de estado (EoS) gas ideal**: \(U=[\rho,\rho u,\rho E]\), \(u = U[1]/U[0]\), \(p = (\gamma - 1)\big(U[2] - 0.5\,U[1]^2/U[0]\big)\).
...
- **Energía neta**: \(dQ_{net} = \eta_{comb}\,dQ_{Wiebe} - dQ_{loss}\); paso de actualización: \(p = p_{adiab} + (\gamma-1) dQ_{net}/V\) como discretización del balance de energía interna.
...
- **Fricción y accesorios**: FMEP con coeficientes en `Friction` y multiplicador `global_scaling_factor`; se suma torque de accesorios para obtener par de fricción total. Par efectivo = Par indicado − Par fricción − pérdidas de bombeo.
...
- **Condiciones de frontera explícitas**:
  - **Inlet con válvula (isentrópico)**: se calcula flujo másico/entalpía vía tobera isentrópica usando \(P_{stag}, T_{stag}\) del cilindro (condiciones de estancamiento de la rama 0D) y \(P_{static}\) de la celda 0. Criterio: si \(P_{static}/P_{stag} < P_{crit}\) con \(P_{crit} = (\tfrac{\gamma+1}{2})^{\gamma/(\gamma-1)}\) → flujo ahogado; los términos de masa/energía se inyectan en la celda 0.
  - **Inlet cerrado**: celda fantasma reflectiva \(\rho_0=\rho_1\), \((\rho u)_0 = - (\rho u)_1\), \((\rho E)_0 = (\rho E)_1\).
  - **Outlet abierto**: presión estática fija \(P_{amb}\) en la celda fantasma; si \(u_{internal}>0\) (outflow) \(T_{ghost}=T_{internal}\); si \(u_{internal}<0\) (inflow) \(T_{ghost}=T_{amb}\) con \(T_{amb}=air\_temperature\_c+273.15\). No se usa condición transmisiva sin presión fija para preservar reflexiones.
- **Junctions**: \(\tfrac{dm}{dt} = \sum \dot m\), \(\tfrac{d(me)}{dt} = \sum (\dot m h_{tot})\); actualización de \(p,T\) mediante EoS en volumen constante.
  - **Fuente de fricción**: \(S_{mom} = -\tfrac{f}{2D}\,\rho u|u|\); \(S_E = u\,S_{mom}\) se aplica como término fuente en la ecuación conservativa de energía.
...
- **Identidades físicas**: Potencia-torque: \(P_{kW} = T_{Nm}\,RPM/9549\), \(P_{hp} = T_{Nm}\,RPM/7127\) (hp mecánico, usado en `mean_power_hp`); BMEP↔Torque (4T: \(BMEP = 4\pi T/V_{disp}\)). FMEP usa RPM en rev/min y presiones absolutas.
