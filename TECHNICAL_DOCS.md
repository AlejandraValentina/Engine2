...
- **Ecuación de estado (EoS) gas ideal**: \(U=[\rho,\rho u,\rho E]\), \(p = (\gamma - 1)\big(U[2] - 0.5\,\rho u^2\big)\).
...
- **Dominio angular 0–720° CA** para cada ciclo: 0° TDC solape, 180° BDC admisión/escape, 360° TDC compresión, 540° BDC expansión, 720° TDC inicio nuevo ciclo.

## 2. Modelo 0D – Termodinámica (core/thermo.py)
- **Desacoplo del 1D**: el solver 0D calcula par/potencia sin retroalimentación del solver 1D. El escape se modela con una contrapresión heurística \(P_{amb}\times `exhaust_backpressure_factor`\) (default 1.05).
...
- **Energía neta**: \(dQ_{net} = \eta_{comb}\,dQ_{Wiebe} - dQ_{loss}\); presión de potencia \(p = p_{adiab} + (\gamma-1) dQ_{net}/V\).
...
- **Condiciones de frontera explícitas**:
  - **Inlet con válvula (isentrópico)**: se calcula flujo másico/entalpía vía tobera isentrópica usando \(P_{stag}, T_{stag}\) del cilindro (traza 0D) y \(P_{static}\) de la celda 0. Si \(P_{static}/P_{cyl} < P_{crit}\) → flujo ahogado; los términos de masa/energía se inyectan en la celda 0.
  - **Inlet cerrado**: celda fantasma reflectiva \(\rho_0=\rho_1\), \((\rho u)_0 = - (\rho u)_1\), \((\rho E)_0 = (\rho E)_1\).
  - **Outlet abierto**: presión estática fija \(P_{amb}\) en la celda fantasma; \(T_{ghost} = T_{internal}\) si hay salida. No se usa condición transmisiva sin presión fija para preservar reflexiones.
...
- **Identidades físicas**: HP↔Torque (\(HP = T\,RPM/9549\)), BMEP↔Torque (4T: \(BMEP = 4\pi T/V_{disp}\)). FMEP usa RPM en rev/min y presiones absolutas.
