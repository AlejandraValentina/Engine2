...
- **Combustión (Wiebe)**: \(x(\theta)=1-\exp\{-a\big(\tfrac{\theta-\theta_{start}}{\Delta\theta}\big)^{m+1}\}\) con \(a,m\) tomados de `Combustion.wiebe_a/m`. Calor químico \(Q_{chem} = m_{fuel}\,LHV\); eficiencia de combustión \(\eta_{comb}\) aplicada antes de pérdidas térmicas. Overrides opt-in: `combustion.wiebe` puede fijar `ca50_deg_atdc`, `burn_duration_deg` y `start/end_deg_atdc`; si `eta_scale` está presente se multiplica \(\eta_{comb}\) (clamp a 1.0) sólo cuando Wiebe está habilitado.
...
- **Capacidad por CFM (port flow)**: `Head.port_flow_cfm` es **por válvula de admisión** a 28 inH2O. El modelo calcula `head_supply_cfm = port_flow_cfm * intake_valves * num_cylinders` y luego `supply_cfm = min(head_supply_cfm, throttle_cfm)` para limitar VE.
...
- **Fuentes geometricas (A(x))**: S_rho = -(rho*u)*(dA_dx/A), S_mom = -(rho*u^2)*(dA_dx/A), S_E = -(u*(rhoE + p))*(dA_dx/A).
...
- **Condiciones de frontera explícitas**:
  - **Inlet con válvula (isentrópico)**: se calcula flujo másico/entalpía vía tobera isentrópica usando \(P_{stag}, T_{stag}\) del cilindro/puerto (condiciones de estancamiento entregadas por el solver 0D) y \(P_{static}\) de la celda 0. Criterio: si \(P_{static}/P_{stag} \le P_{crit}\) con \(P_{crit} = (\tfrac{2}{\gamma+1})^{\gamma/(\gamma-1)}\) → flujo ahogado; los términos de masa/energía se inyectan en la celda 0. En el modelo actual 0D no se resuelve velocidad de puerto (\(u_{port}=0\)), por lo que \(P_{stag}=P_{cyl}\) y \(T_{stag}=T_{cyl}\) (totales = estáticos en el upstream 0D).
    - **Área de válvula real**: \(A_{curtain} = \pi\,D_{seat}\,\text{lift}\,N_{valves}\) con `Camshaft.get_lift` (wrap 0–720) y \(D_{seat}\) de `CylinderHead`. `compute_exhaust_valve_area` devuelve solo el área geométrica; el área efectiva es \(A_{eff} = C_d A_{valve}\) con `exhaust_valve_cd` aplicado una sola vez en la boquilla. Alternativamente, `exhaust_valve_area_model="fixed"` usa el área de asiento \(A_{seat} = \pi (D_{seat}/2)^2\). Si faltan datos de cam/cabeza, se usa el placeholder sinusoidal.
