...
- **Ecuaciones**: Euler 1D inviscid con términos fuente (Darcy–Weisbach): \(S_{mom} = -\tfrac{f}{2D}\,\rho u|u|\), \(S_E = u\,S_{mom}\); sin transferencia de calor (adiabático). **Nota de implementación actual**: `numerics.source_terms` implementa la fricción como `friction = -0.5*rho*u*abs(u)*f/D`, pero `lax_wendroff_step` pasa \(D=1.0\) fijo, por lo que el diámetro no escala el término de fricción aún; el diámetro influye vía el perfil de área \(A(x)\) y las áreas de frontera.
