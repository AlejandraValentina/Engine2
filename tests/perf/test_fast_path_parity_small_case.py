...
def test_fast_path_parity_small_case() -> None:
    engine = Engine()
    baseline = run_full_scope(engine, duration_s=0.01, max_steps=80, target_dx=0.05, use_numba=False)
    fast = run_full_scope(engine, duration_s=0.01, max_steps=80, target_dx=0.05, use_numba=True)
