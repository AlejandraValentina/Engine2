...
def test_curtain_area_zero_when_closed_non_wrap() -> None:
    cam = Camshaft(exhaust_lift=10.0, exhaust_duration=200.0, lobe_separation=110.0, advance=0.0)
...
def test_curtain_area_wrap_has_flow_at_zero() -> None:
    ...
    area_zero = compute_exhaust_valve_area(0.0, cam, head, settings)
    area_wrap = compute_exhaust_valve_area(720.0, cam, head, settings)

    assert area_zero > 0.0
    assert area_wrap == pytest.approx(area_zero, rel=1e-12)


def test_curtain_area_scales_with_lift() -> None:
    cam_low = Camshaft(exhaust_lift=5.0, exhaust_duration=200.0)
    cam_high = Camshaft(exhaust_lift=12.0, exhaust_duration=200.0)
    head = CylinderHead(exhaust_valves=1, exhaust_valve_diameter_mm=32.0)
    settings = SimulationSettings(exhaust_valve_area_model="curtain", exhaust_valve_cd=1.0)
