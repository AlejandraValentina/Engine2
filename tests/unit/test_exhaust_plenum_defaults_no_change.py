...
from core.advanced.plenum_cv import ExhaustPlenumConfig


def test_exhaust_plenum_defaults_no_change(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cfg_plenum_off = orchestrator_module.OrchestratorConfig(
        **base_cfg,
        exhaust_plenum=ExhaustPlenumConfig(enabled=False),
    )
    ...
    result_plenum_off = orchestrator_module.Orchestrator(cfg_plenum_off).run(
        rpm=2000.0,
        pipe_cells=2,
        pipe_length_m=0.2,
        pipe_diameter_m=0.05,
    ...
    for key in ("angle_deg", "pressure", "ve", "trapped_mass", "indicated_work", "periodicity_metric"):
        assert np.allclose(result_baseline[key], result_plenum_off[key], atol=0.0)
    assert result_baseline["convergence_history"] == result_plenum_off["convergence_history"]
