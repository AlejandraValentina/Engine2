...
def test_sweep_produces_monotonic_rpm_grid(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    rpms = [entry["rpm"] for entry in sweep_results]
    assert rpms == sorted(rpms)
    assert rpms == [1000.0, 1500.0, 2000.0]
